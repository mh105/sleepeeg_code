...
% We found some interesting patterns in the AD Sleep project resting state,
% where the young adults have a band of low alpha power during eyes closed
% period under common average referencing. Is this replicated in the NSS
% dataset with 65 channels? 
...
dataDir = '/Users/alexhe/Dropbox_Personal/Mac_Desktop/MIT_HST/NSRL_Purdon_Prerau/NSS_related';

% addpath to artifact detection folder
refDir = '/Users/alexhe/Dropbox_Personal/Mac_Desktop/MIT_HST/NSRL_Purdon_Prerau/reference_modeling';
...
% look for an eyes-closed period with occipital alpha
load(fullfile(dataDir, 'NSS11_1_full.mat'))
load(fullfile(refDir, 'util_struct', 'eloc_structure.mat'))
Fs = 500;

% Plot the spectrogram on an occipital channel

topoplot([],eloc_new,'style','both','electrodes','ptsnumbers','emarker', {'.', 'k', 30, 1});

% common average re-reference
ca = nanmean(data(1:64,:),1);
allchan = data(1:64,:) - ca;

% pick an occipital channel: 47
channel = 47;
data = allchan(channel,:);

[mt_spectrogram, stimes, sfreqs] = multitaper_spectrogram_optimized(data,...
    Fs, [0, (Fs/2)], [2,3], [5,1], 2^16, 'linear', false, true);
        
quick_spectrogram(mt_spectrogram, stimes, sfreqs)

% good segment is found for eyes closed period 
xlimit = [1430, 1565];
xlim(xlimit)
ec_data = data(times > xlimit(1) & times <= xlimit(2));
t = times(times > xlimit(1) & times <= xlimit(2));
...
ax1 = subplot(2,1,1);
plot(t, ec_data, 'LineWidth', 1)
hold on
title(['EEG', num2str(channel), '-CA Signal Time Trace'])
xlabel('Time (min)')
ylabel('Voltage (\mu V)')
set(gca, 'FontSize', 20)
...
%Detect artifacts
[artifacts, hf_artifacts, bb_artifacts, y_high, y_broad] = EEG_detect_time_domain_artifacts(ec_data,...
    Fs, method, hf_crit, hf_pass, bb_crit, bb_pass, smooth_duration, verbose, histogram_plot);

ax2 = subplot(2,1,2);
plot(t, artifacts, 'LineWidth', 2)
title(['EEG', num2str(channel), '-CA Artifact Detection'])
xlabel('Time (min)')
...
%% Spectrum estimate 
[mt_spectrogram, stimes, sfreqs] = multitaper_spectrogram_optimized(ec_data,...
    Fs, [0, (Fs/2)], [2,3], [5,1], 2^16, 'linear', false, true);
[mt_spectrogram1, stimes1, sfreqs1] = multitaper_spectrogram_optimized(ec_data,...
    Fs, [0, (Fs/2)], [2,3], [10,1], 2^16, 'linear', false, true);
[mt_spectrogram2, stimes2, sfreqs2] = multitaper_spectrogram_optimized(ec_data,...
...
xlabel('Frequency (Hz)')
ylabel('PSD (dB)')
title({'Multitaper Spectral Estimation'}, 'interpreter', 'none')
set(gca, 'FontSize', 20)
...
for k = 1:length(chanlocs)
    ...
    % grab channel
    s = allchan(k, times > xlimit(1) & times <= xlimit(2));
    assert(length(s)/Fs == diff(xlimit), 'Incorrect length of eyes closed.')
    %Detect artifacts
