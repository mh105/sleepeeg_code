...
assert((ndims(EEG.data) == 3) || (ndims(EEG.data) == 2),['Input EEG data matrix "%s.data" must be an epoched 3-dimensional channels\n',...
    ...
    'matrix.'], inputname(EEGStructInd)) %#ok<*ISMAT>
if ndims(EEG.data) == 2
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Memory optimization downsample.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if strcmpi(EBinput.EEGtype,'Continuous')
    % Detect constant channels and set them to zero for filtering
    % only detect LM and RM channels to save time.
    [ ~, ~, EEG ] = SleepEEG_constantChan(EEG, 0.95, true, [1, 47]);
    disp(' ')
end

% Detect signal sampling rate and downsample if more than 100Hz
if EEG.srate > 100
    % we change the data precision to single to conserve memory space
    if ~isa(EEG.data, 'single')
        EEG.data = single(EEG.data);
    end
    % we downsample to 100Hz before passing to eBridge
    disp('SleepEEG: To save memory, we downsample to 100Hz before bridging analysis.')
    disp(['SleepEEG: Note that the data outside eBridge() is kept at its original sampling rate: ' num2str(EEG.srate) 'Hz'])
    disp('SleepEEG: Downsampling to 100Hz...')
    EEG = pop_resample(EEG, 100, 0.9, 0.1);
    disp(' ')
    disp('SleepEEG: Epoch length in eBridge is updated to 100 to make it 1s long.')
    EBinput.EpLength = 100;
    disp(' ')
end
...
if strcmpi(EBinput.EEGtype,'Continuous')
    ...
    if EBinput.FiltMode == 1
        ...
        try
            EEG = pop_eegfilt(EEG,0,EB.Info.FiltUpper,[],[],1,0,'fir1',0); % this uses fft filter
            EEG = pop_eegfilt(EEG,EB.Info.FiltLower,0,[],[],1,0,'fir1',0);
...
% % Depending on whether some channels were excluded, we can use parfor or
% % regular for loop.
% if EEG.pnts > EEG.srate*60*60 % signal is longer than an hour
%     useparfor = true;
% else % otherwise it's not worth using parfor
%     useparfor = false;
% end

if LoopLen == EEG.nbchan % this allows straight-forward parfor
    fprintf([mBeg 'Using parfor to compute ED matrix.\n'])
    totLoops = LoopLen-1;
    fulldata = EEG.data;
    ...
    for a = 1:(LoopLen-1)
        ...
        parfor b = (a+1):LoopLen
            %Make difference matrix, diffE (.NumChans x .NumChans).
            diffE = squeeze(current_a - fulldata(b,:,:));
            %Create ED values and add to ED.
            ED(a,b,:) = var(diffE,1,1);
        end
    end
    
else % use serial for loops
    fprintf([mBeg 'Using serial for loops to compute ED matrix.\n'])
    totLoops = ((EB.Info.NumChans.^2) - EB.Info.NumChans) / 2;
...
% fprintf([repmat(sprintf('\b'), 1, numDots+2+length(mBeg))]); %#ok<NBRAK>
disp(' ')
...
EDcounts = histc(EDvect,binEdges); %#ok<HISTC>
...
if (EBinput.PlotMode == 2) || ((EBinput.PlotMode == 1) && (EB.Bridged.Count ~= 0))
    ...
    %Plot a magnified look at the [0 6] region --- added by AlexHe
    subplot(1,2,2)
    plot(binCentersSPL, EDcountsSPL, 'b-')
    set(gca,'XLim',[0 binMax]);
    %Plot EDcutoff.
    ...
    %Add title and axis labels.
    set(gca,'XLim',[0 6]);
    title('ED distribution Expanded for [0, 6]');
    xlabel('Electrical Distance (ED)');
    ylabel('Number of epochs');
else
    EB.Info.FigHandle = [];
